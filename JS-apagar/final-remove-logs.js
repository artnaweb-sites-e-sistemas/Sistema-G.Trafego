...
// Função para remover console.log de forma inteligente
function removeConsoleLogsIntelligently(filePath) {
  try {
    ...
    // Regex mais inteligente que preserva a estrutura
    // Remove console.log simples (uma linha)
    ...
    // Remove console.log que estão no meio de outras linhas (mas preserva a estrutura)
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        if (extensions.includes(ext)) {
          processedCount++;
          if (removeConsoleLogsIntelligently(fullPath)) {
            modifiedCount++;
          }
        }
      }
