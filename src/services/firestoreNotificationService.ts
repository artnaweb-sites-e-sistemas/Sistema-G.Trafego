...
class FirestoreNotificationService {
  ...
  /**
   * Sanitizar strings para uso seguro em IDs do Firestore
   */
  private sanitizeForFirestore(text: string): string {
    return text
      .replace(/[|\/\\]/g, '_') // Substituir pipes, barras por underscore
      .replace(/\s+/g, '_') // Substituir espaços por underscore
      .replace(/[[\]]/g, '') // Remover colchetes
      .replace(/[()]/g, '') // Remover parênteses
      .replace(/[^a-zA-Z0-9_-]/g, '') // Remover outros caracteres especiais
      .substring(0, 50); // Limitar tamanho para não ficar muito longo
  }
  ...
  async saveNotification(
    ...
    try {
      const clientSafe = this.sanitizeForFirestore(metadata.client || 'system');
      const productSafe = this.sanitizeForFirestore(metadata.product || 'general');
      const notificationId = `${userId}_${type}_${clientSafe}_${productSafe}_${Date.now()}`;
  ...
  async getUserNotifications(userId: string): Promise<NotificationRecord[]> {
    try {
      // 🎯 TEMPORÁRIO: Remover ordenação até criar o índice
      const q = query(
        collection(db, this.collection),
        where('userId', '==', userId)
        // orderBy('createdAt', 'desc') // Comentado temporariamente
      );
      ...
      // 🎯 TEMPORÁRIO: Ordenar manualmente até criar o índice
