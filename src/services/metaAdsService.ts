...
class MetaAdsService {
  ...
  // Sistema de rate limiting REMOVIDO - sem limites

  // Métodos de rate limit REMOVIDOS - sem limites
  ...
  // Métodos de verificação de rate limit REMOVIDOS - sempre retorna true
  private async canAttemptOAuth(): Promise<boolean> {
    return true; // Sempre permite tentativas
  }

  private recordOAuthAttempt(): void {
    // Não faz nada - sem rate limit
  }
  ...
  // 🎯 NOVA FUNÇÃO: Emitir evento de rate limit para notificar componentes
  private emitRateLimitEvent(message: string): void {
    try {
      ...
      console.warn('Erro ao emitir evento de rate limit:', error);
    }
  }

  // Métodos de verificação de status de rate limit REMOVIDOS - sempre retorna status livre
  ...
  // Verificar se existem campanhas em um período específico (requisição mínima)
  async hasCampaignsInPeriod(clientName: string, period: string): Promise<boolean> {
    try {
      // Converter período para datas (ex: "Maio 2025" -> dateStart e dateEnd)
      ...
      const dateStart = `${year}-${String(monthIndex + 1).padStart(2, '0')}-01`;
      const lastDay = new Date(year, monthIndex + 1, 0).getDate();
      const dateEnd = `${year}-${String(monthIndex + 1).padStart(2, '0')}-${lastDay}`;
  ...
  async getAdSetInsights(adSetId: string, dateStart: string, dateEnd: string, options?: { fallbackToLast30Days?: boolean }): Promise<MetaAdsInsight[]> {
    ...
    try {
      ...
      if (error.response?.status === 400) {
        // Erro 400 - geralmente conjunto novo sem dados ou parâmetros inválidos
        console.info(`ℹ️ AdSet ${adSetId} não tem dados de insights disponíveis (pode ser conjunto novo)`);
        return []; // Retornar array vazio ao invés de erro
      } else if (error.response?.status === 429) {
        // Rate limit excedido
        console.warn(`⚠️ Rate limit excedido para adSet ${adSetId}, tentando novamente...`);
        throw new Error(`Rate limit excedido. Tente novamente em alguns minutos.`);
      } else {
        // Outros erros
        console.error(`❌ Erro ao buscar insights para adSet ${adSetId}:`, error?.response?.data || error.message);
        throw new Error(`Erro ao buscar insights do conjunto de anúncios: ${error.response?.data?.error?.message || error.message}`);
      }
    }
  }

  // 🎯 NOVO MÉTODO: Buscar insights consolidados por mês
  async getAdSetInsightsMonthly(adSetId: string, dateStart: string, dateEnd: string): Promise<MetaAdsInsight[]> {
    if (!this.user) {
      throw new Error('Usuário não está logado. Faça login primeiro.');
    }
    ...
    try {
      ...
      const response = await axios.get(
        `${this.baseURL}/${adSetId}/insights`,
        {
          params: {
            ...
            time_increment: 'monthly' // 🎯 DADOS JÁ CONSOLIDADOS POR MÊS
          }
        }
      );
      ...
      if (error.response?.status === 400) {
        console.info(`ℹ️ AdSet ${adSetId} não tem dados de insights mensais disponíveis`);
        return [];
      } else if (error.response?.status === 429) {
        console.warn(`⚠️ Rate limit excedido para adSet ${adSetId}`);
        throw new Error(`Rate limit excedido. Tente novamente em alguns minutos.`);
      } else {
        console.error(`❌ Erro ao buscar insights mensais para adSet ${adSetId}:`, error?.response?.data || error.message);
        throw new Error(`Erro ao buscar insights mensais: ${error.response?.data?.error?.message || error.message}`);
      }
    }
  }
  ...
  convertToMetricData(insights: MetaAdsInsight[], month: string, client?: string, product?: string, audience?: string): any[] {
    const result = insights.map(insight => {
      ...
      if (parseInt(messagingConversations) > 0 && parseFloat(costPerMessagingConversation) > 0) {
        ...
        // Calcular CPL manualmente: investimento / leads
        cpl = leadsCount > 0 ? investment / leadsCount : 0;
      }
  ...
  // Buscar adsets ativos filtrados por produto
  async getActiveAdSetsByProduct(product: string): Promise<any[]> {
    if (!this.selectedAccount) {
      return [];
    }
  ...
  // Buscar insights de adsets ativos por produto
