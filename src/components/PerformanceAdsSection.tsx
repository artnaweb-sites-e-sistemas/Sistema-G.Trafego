...
interface AdMetrics {
  ...
  last7DaysDetails?: Array<{ date: string; cpr: number; cpc: number; ctr: number; frequency: number }>;
...
const PerformanceAdsSection: React.FC<PerformanceAdsSectionProps> = ({ onBack }) => {
  ...
  const fetchRealAdsData = async () => {
    ...
    try {
      ...
      // 🎯 NOVA FUNÇÃO: Calcular últimos 7 dias baseado no último dia com gasto do anúncio
      const getLast7DaysFromLastActiveDate = async (adId: string, month: string) => {
        try {
          const { startDate, endDate } = getPeriodDates(month);
          
          // Buscar insights de todo o período para encontrar o último dia com gasto
          const allInsights = await metaAdsService.getAdInsights(adId, startDate, endDate, false);
          
          if (!allInsights || allInsights.length === 0) {
            // Se não há dados, usar a lógica padrão
            return getLast7DaysDates(month);
          }
          
          // Encontrar o último dia com gasto > 0
          const daysWithSpend = allInsights
            .filter((insight: any) => parseFloat(insight.spend || '0') > 0)
            .map((insight: any) => insight.date_start)
            .sort((a: string, b: string) => new Date(b).getTime() - new Date(a).getTime());
          
          if (daysWithSpend.length === 0) {
            // Se não há dias com gasto, usar a lógica padrão
            return getLast7DaysDates(month);
          }
          
          // Último dia com gasto
          const lastActiveDate = new Date(daysWithSpend[0]);
          
          // Calcular 7 dias anteriores a partir do último dia ativo
          const last7DaysStart = new Date(lastActiveDate);
          last7DaysStart.setDate(lastActiveDate.getDate() - 6); // 7 dias atrás (incluindo o último dia ativo)
          
          return {
            startDate: last7DaysStart.toISOString().split('T')[0],
            endDate: lastActiveDate.toISOString().split('T')[0]
          };
          
        } catch (error) {
          console.error('Erro ao calcular últimos 7 dias do último período ativo:', error);
          // Fallback para lógica padrão
          return getLast7DaysDates(month);
        }
      };

      // 🎯 NOVA FUNÇÃO: Calcular últimos 3 dias baseado no último dia com gasto do anúncio
      const getLast3DaysFromLastActiveDate = async (adId: string, month: string) => {
        try {
          const { startDate, endDate } = getPeriodDates(month);
          ...
          if (!allInsights || allInsights.length === 0) {
            // Se não há dados, usar a lógica padrão
            return getLast3DaysDates(month);
          }
          ...
          // Calcular 3 dias anteriores a partir do último dia ativo
          const last3DaysStart = new Date(lastActiveDate);
          last3DaysStart.setDate(lastActiveDate.getDate() - 2); // 3 dias atrás (incluindo o último dia ativo)
          ...
          console.error('Erro ao calcular últimos 3 dias do último período ativo:', error);
          // Fallback para lógica padrão
          return getLast3DaysDates(month);
        }
      };

      // 🎯 NOVA FUNÇÃO: Calcular 7 dias anteriores baseado no último dia com gasto do anúncio
      const getPrevious7DaysFromLastActiveDate = async (adId: string, month: string) => {
        try {
          const { startDate, endDate } = getPeriodDates(month);
          ...
          if (!allInsights || allInsights.length === 0) {
            // Se não há dados, usar a lógica padrão
            return getPrevious7DaysDates(month);
          }
          ...
          // Calcular 7 dias anteriores ao período dos últimos 7 dias
          const previous7DaysEnd = new Date(lastActiveDate);
          previous7DaysEnd.setDate(lastActiveDate.getDate() - 7);
          ...
          console.error('Erro ao calcular 7 dias anteriores do último período ativo:', error);
          // Fallback para lógica padrão
          return getPrevious7DaysDates(month);
        }
      };

      // 🎯 NOVA FUNÇÃO: Calcular 3 dias anteriores baseado no último dia com gasto do anúncio
      const getPrevious3DaysFromLastActiveDate = async (adId: string, month: string) => {
        try {
          const { startDate, endDate } = getPeriodDates(month);
          ...
          if (!allInsights || allInsights.length === 0) {
            // Se não há dados, usar a lógica padrão
            return getPrevious3DaysDates(month);
          }
          ...
          // Calcular 3 dias anteriores ao período dos últimos 3 dias
          const previous3DaysEnd = new Date(lastActiveDate);
          previous3DaysEnd.setDate(lastActiveDate.getDate() - 3);
          ...
          console.error('Erro ao calcular 3 dias anteriores do último período ativo:', error);
          // Fallback para lógica padrão
          return getPrevious3DaysDates(month);
        }
      };
        ...
        try {
          ...
          try {
            ...
            if (ad.created_time) {
              ...
              // 🎯 CORREÇÃO: Buscar insights dos últimos 7 dias baseado no último dia com gasto
              const { startDate: activeLast7Start, endDate: activeLast7End } = await getLast7DaysFromLastActiveDate(ad.id, selectedMonth);
              last7DaysInsights = await metaAdsService.getAdInsights(ad.id, activeLast7Start, activeLast7End, false);
              
              
              // 🎯 CORREÇÃO: Buscar insights dos últimos 3 dias baseado no último dia com gasto
              const { startDate: activeLast3Start, endDate: activeLast3End } = await getLast3DaysFromLastActiveDate(ad.id, selectedMonth);
              last3DaysInsights = await metaAdsService.getAdInsights(ad.id, activeLast3Start, activeLast3End, false);
              
              
              // 🎯 CORREÇÃO: Buscar insights dos 7 dias anteriores baseado no último dia com gasto
              const { startDate: activePrevious7Start, endDate: activePrevious7End } = await getPrevious7DaysFromLastActiveDate(ad.id, selectedMonth);
              previous7DaysInsights = await metaAdsService.getAdInsights(ad.id, activePrevious7Start, activePrevious7End, false);
              
              
              // 🎯 CORREÇÃO: Buscar insights dos 3 dias anteriores baseado no último dia com gasto
              const { startDate: activePrevious3Start, endDate: activePrevious3End } = await getPrevious3DaysFromLastActiveDate(ad.id, selectedMonth);
              previous3DaysInsights = await metaAdsService.getAdInsights(ad.id, activePrevious3Start, activePrevious3End, false);
          ...
          if (insights.length > 0) {
            ...
            if (last7DaysInsights.length > 0) {
              ...
              last7DaysInsights.forEach((insight: any) => {
                ...
                // 🎯 CORREÇÃO: Criar dados para o tooltip dos últimos 7 dias
                if (!last7DaysDetails) last7DaysDetails = [];
  ...
  const getTrendIcon = (trend: string | null) => {
    switch (trend) {
      ...
      case 'stable-positive':
        return (
          <div className="flex items-center space-x-1" title="Performance estável e boa">
            <BarChart3 className="w-4 h-4 text-emerald-400" />
            <div className="w-1 h-1 bg-emerald-400 rounded-full"></div>
          </div>
        );
      case 'stable-negative':
        return (
          <div className="flex items-center space-x-1" title="Performance estável mas ruim">
            <BarChart3 className="w-4 h-4 text-orange-400" />
            <div className="w-1 h-1 bg-orange-400 rounded-full"></div>
          </div>
        );
      case 'stable': // Fallback para compatibilidade
  ...
  const calculateTrendIcon = ({
    ...
    tipoConversao?: string;
    performanceScore?: number;
  }): { trend: 'up' | 'down' | 'warning' | 'stable-positive' | 'stable-negative' | 'collecting'; explanation: string } => {
    // === DETECÇÃO DE ANÚNCIOS SEM DADOS SUFICIENTES ===
    // Verificar se o anúncio não está em veiculação nos últimos 7 dias (período ativo)
    ...
    // === TENDÊNCIA PRINCIPAL (Últimos 7 dias do período ativo) ===
    ...
    const frequenciaControlada7Dias = frequenciaAtual <= 3.5; // Não saturando
    ...
    const condicoesPositivas = [cprMelhorou, conversoesAumentaram, ctrMelhorou, frequenciaControlada7Dias].filter(Boolean).length;
    ...
    // === ALERTA CRÍTICO DE 1 DIA (Detecção ultrarrápida) ===
    // 🚨 NOVO: Detectar quedas críticas no último dia comparado com média dos 3 dias anteriores
    // Isso permite alerta mais rápido sem esperar 3 dias de dados ruins
    const mediaCPR3DiasAnteriores = previous3DaysCpr;
    const mediaCTR3DiasAnteriores = previous3DaysCtr;
    const mediaConversoes3DiasAnteriores = previous3DaysConversions;
    
    // Alerta crítico: último dia muito pior que a média dos 3 dias anteriores
    const alertaCritico1DiaCPR = mediaCPR3DiasAnteriores > 0 && last3DaysCpr > mediaCPR3DiasAnteriores * 1.5; // 50% pior em 1 dia
    const alertaCritico1DiaCTR = mediaCTR3DiasAnteriores > 0 && last3DaysCtr < mediaCTR3DiasAnteriores * 0.7; // 30% pior em 1 dia
    const alertaCritico1DiaConversoes = mediaConversoes3DiasAnteriores > 0 && last3DaysConversions < mediaConversoes3DiasAnteriores * 0.5; // 50% menos em 1 dia
    const alertaCritico1DiaFrequencia = frequenciaAtual > 6.0; // Frequência crítica
    
    const alertasCriticos1Dia = [alertaCritico1DiaCPR, alertaCritico1DiaCTR, alertaCritico1DiaConversoes, alertaCritico1DiaFrequencia].filter(Boolean).length;
    ...
    // 0. 🚨 ALERTA CRÍTICO: Queda brusca em 1 dia (prioridade máxima)
    if (alertasCriticos1Dia >= 1) {
      const reasons = [];
      if (alertaCritico1DiaCPR) {
        const variacao = ((last3DaysCpr - mediaCPR3DiasAnteriores) / mediaCPR3DiasAnteriores * 100);
        reasons.push(`${tipoConversao} dispararam ${variacao.toFixed(1)}% no último dia ativo vs média anterior`);
      }
      if (alertaCritico1DiaCTR) {
        const variacao = ((mediaCTR3DiasAnteriores - last3DaysCtr) / mediaCTR3DiasAnteriores * 100);
        reasons.push(`CTR despencou ${variacao.toFixed(1)}% no último dia ativo vs média anterior`);
      }
      if (alertaCritico1DiaConversoes) {
        const variacao = ((mediaConversoes3DiasAnteriores - last3DaysConversions) / mediaConversoes3DiasAnteriores * 100);
        reasons.push(`${tipoConversao} caíram ${variacao.toFixed(1)}% no último dia ativo vs média anterior`);
      }
      if (alertaCritico1DiaFrequencia) {
        reasons.push(`público supersaturado no último dia ativo (${frequenciaAtual.toFixed(1)}x)`);
      }
      
      return {
        trend: 'warning',
        explanation: `🚨 ALERTA CRÍTICO - ${objetivo.toLowerCase()}!<br>${reasons.join('<br>')}.<br>Intervenção imediata necessária!`
      };
    }
    ...
    // 4. ESTÁVEL POSITIVO vs ESTÁVEL NEGATIVO
    // 🆕 MELHORIA: Distinguir entre estável com boa performance vs estável com performance ruim
    
    // Critérios de desempenho ruim:
    // - score geral baixo (ex.: < 45)
    // - sem conversões com gasto (> 0) nos últimos 7 dias do período ativo
    const gastoSemConversao = cprAtual > 0 && conversoesAtuais === 0;
    const scoreBaixo = (performanceScore || 0) < 45;
    const performanceRuim = gastoSemConversao || scoreBaixo;
    
    // Critérios de boa performance:
    const scoreBom = (performanceScore || 0) >= 65;
    const temConversoes = conversoesAtuais > 0;
    const ctrRazoavel = ctrAtual >= 1.0; // CTR mínimo aceitável
    const frequenciaControlada = frequenciaAtual <= 4.0;
    const performanceBoa = scoreBom && temConversoes && ctrRazoavel && frequenciaControlada;
    
    if (performanceRuim) {
      return {
        trend: 'stable-negative',
        explanation: `${objetivo} estável, mas com performance ruim.<br>Sem mudanças significativas, porém resultados baixos. Otimização recomendada.`
      };
    }
    ...
    if (performanceBoa) {
      return {
        ...
        explanation: `${objetivo} estável com boa performance!<br>Mantendo resultados consistentes e saudáveis. Continue monitorando.`
      };
    }
    
    // Caso intermediário: nem muito bom, nem muito ruim
    return {
      trend: 'stable-positive',
      explanation: `${objetivo} estável.<br>Performance moderada sem grandes mudanças nos últimos dias.`
    };
  };
                  ...
                  <div className="flex items-center gap-2">
                      ...
                      {(() => {
                        const trendData = calculateTrendIcon({
                          // 🎯 CORREÇÃO: Usar dados dos últimos 7 dias do período ativo para análise de tendência
                          ...
                          // Manter dados dos 3 dias para alertas imediatos
                          ...
                          performanceScore: ad.performanceScore || 0
                        });
                                   ...
                                   <div className="space-y-2">
                                     ...
                                     <div className="flex items-center gap-2 mb-2">
                                       <div className={`w-2 h-2 rounded-full ${
                                         trendData.trend === 'warning' ? 'bg-red-400 animate-pulse' :
                                         trendData.trend === 'up' ? 'bg-green-400' :
                                         trendData.trend === 'down' ? 'bg-red-400' :
                                         trendData.trend === 'stable-positive' ? 'bg-emerald-400' :
                                         trendData.trend === 'stable-negative' ? 'bg-orange-400' :
                                         trendData.trend === 'collecting' ? 'bg-amber-400 animate-pulse' : 'bg-slate-400'
                                       }`}></div>
                                       <span className={`font-semibold text-xs uppercase tracking-wide ${
                                         trendData.trend === 'warning' ? 'text-red-300' :
                                         trendData.trend === 'up' ? 'text-green-300' :
                                         trendData.trend === 'down' ? 'text-red-300' :
                                         trendData.trend === 'stable-positive' ? 'text-emerald-300' :
                                         trendData.trend === 'stable-negative' ? 'text-orange-300' :
                                         trendData.trend === 'collecting' ? 'text-amber-300' : 'text-slate-300'
                                       }`}>
                                         {trendData.trend === 'warning' ? 'Alerta para Conversões!' : 
                                          trendData.trend === 'up' ? 'Tendência Positiva!' :
                                          trendData.trend === 'down' ? 'Tendência Negativa!' : 
                                          trendData.trend === 'stable-positive' ? 'Estável - Boa Performance!' :
                                          trendData.trend === 'stable-negative' ? 'Estável - Performance Ruim!' :
                                          trendData.trend === 'collecting' ? 'Coletando Dados!' : 'Status'}
                                       </span>
                                     </div>
                                     ...
                                     {/* Ação recomendada */}
                                     {trendData.trend === 'warning' && trendData.explanation.includes('CRÍTICO') && (
                                     ...
                                     {trendData.trend === 'stable-negative' && (
                                       <div className="mt-2 pt-2 border-t border-slate-600">
                                         <p className="text-xs text-orange-300 font-medium">
                                           🔧 Considere otimizar esta campanha.
                                         </p>
                                       </div>
                                     )}
                                   </div>
          ...
          >
            <div className="bg-slate-900 border border-slate-700 rounded-lg shadow-2xl p-3 w-72 text-slate-200">
              <p className="text-xs text-slate-300 font-semibold mb-2">
                {metricTooltip.metric === 'cpr' && 'Últimos 7 dias do período ativo (CPR/dia)'}
                {metricTooltip.metric === 'cpc' && 'Últimos 7 dias do período ativo (CPC/dia)'}
                {metricTooltip.metric === 'ctr' && 'Últimos 7 dias do período ativo (CTR/dia)'}
                {metricTooltip.metric === 'freq' && 'Últimos 7 dias do período ativo (Frequência/dia)'}
              </p>
